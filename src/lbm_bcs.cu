...
__global__ void gpuApplyInflow(LBMFields d, const int STEP) {
    ...
    float interface_width = 3.0f; // INT_W is a gpu constant, but we can use a different value here for the inflow interface width
    if (radial_dist > radius) return;
        ...
        float profile = 0.5f + 0.5f * tanhf(2.0f * (radius - radial_dist) / INT_W);
        ...
        float phi_in = 0.5f + 0.5f * tanhf(2.0f * (radius - radial_dist) / INT_W);
        #ifdef PERTURBATION
...
__global__ void gpuReconstructBoundaries(LBMFields d) {
    ...
    for (int Q = 0; Q < FLINKS; ++Q) {
        ...
        if (xx >= 0 && xx < NX && yy >= 0 && yy < NY && zz >= 0 && zz < NZ) {
            ...
            d.f[streamed_idx4] = W[Q] * d.rho[idx3];
        }
    }
    #pragma unroll GLINKS
    for (int Q = 0; Q < GLINKS; ++Q) {
        ...
        if (xx >= 0 && xx < NX && yy >= 0 && yy < NY && zz >= 0 && zz < NZ) {
            const int streamed_idx4 = gpuIdxGlobal4(xx,yy,zz,Q);
            d.g[streamed_idx4] = W_G[Q] * d.phi[idx3];
        }
    }
}
